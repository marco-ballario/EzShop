...
public class EZShop implements EZShopInterface {
	...
	private boolean checkDigit(String code) {
		int tmp=0, tot=0;
		int lastCodeDigit = Character.getNumericValue(code.charAt(code.length()-1));  //last code digit, used as check number
		int len = code.length();
		...
		//if check digit is equal to the last digit of the code, that's valid otherwise not
